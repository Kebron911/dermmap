 ...
 * Creates the core original tables (users, patients, visits, lesions, photos,
 * audit_logs, sync_log) in the state they existed before any ALTER TABLE patches.
 * Subsequent migrations add columns and new tables incrementally.
 */

export const up = (pgm) => {
  pgm.sql(`CREATE EXTENSION IF NOT EXISTS "uuid-ossp";`);

  // Users — original columns only (credentials/status/last_login_at/location_id come later)
  pgm.sql(`
  ...
  // Patients — original columns; location_id comes in migration 3
  pgm.sql(`
  ...
  // Photos — original minimal schema; S3 columns, capture_type, BYTEA come in migration 4
  pgm.sql(`
  ...
  // Audit log — HIPAA-required immutable event log (required from day 1)
  pgm.sql(`
